...
# MostActiveStock load

from StockDataAnalysisApp.models import MostActiveStock

def parseMostActiveStock():
...
if __name__=='__main__':
    ...
    StockDatalist = parseMostActiveStock()

    for i in range(len(StockDatalist)):
        MostActiveStock(Symbols=StockDatalist[i][0],Names=StockDatalist[i][1],Prices=StockDatalist[i][2],Changes=StockDatalist[i][3],PercentageChanges=StockDatalist[i][4],MarketCap=StockDatalist[i][5],AverageVolume=StockDatalist[i][6],Volume=StockDatalist[i][7]).save()
